    ...
    for input_file in list_of_input_files:
        ...
        for folder in all_folders:
            if folder == file_path_components[-2]:
                folder_pdb2input_files[folder].append(input_file)
                break
...
pdb_ids = [ file.split(".")[0].split("/")[-1] for file in folder2input_files["wildtype"] ]
...
for idx, pdb_id in enumerate(pdb_ids):
    ...
    for (wildtype_type, mutation_type) in mutations_to_be_scored:
        wildtype_path = folder2input_files[wildtype_type][idx]
